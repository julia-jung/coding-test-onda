...
const bookingOptimizer = require('./booking');
...
MongoClient.connect('mongodb://127.0.0.1:27017', { useNewUrlParser: true }, (error, client) => {
    ...
    app.post('/booking', async (req, res) => {
        try {
            ...
            for(var i = 0; i < optimized.length; i++) {
                const updated = await booking.updateOne({ id: optimized[i].id }, { $set: { bed: optimized[i].bed }});
                if(!updated) {
    ...
    app.delete('/booking/:id', async (req, res) => {
        try {
            const deleted = await booking.deleteMany({ id: parseInt(req.params.id) });
            if(!deleted) {
